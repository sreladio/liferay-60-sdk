...
public class USMemberLocalServiceImpl extends USMemberLocalServiceBaseImpl {
	...
	public List<USMember> getAllMembers(long groupId)
		throws SystemException {
		
		List<USMember> products = usMemberPersistence.findByGroupId(groupId);
		return products;
	}

	public void deleteMemberById(long productId) 
			throws NoSuchMemberException, SystemException, PortalException {

		USMember member = usMemberPersistence.findByPrimaryKey(productId);
		deleteMemberByUSMember(member);
	}
	
	public void deleteMemberByUSMember(USMember member) 
			throws PortalException, SystemException {
		
		resourceLocalService.deleteResource(member.getCompanyId(),
		USMember.class.getName(), ResourceConstants.SCOPE_INDIVIDUAL,
		member.getPrimaryKey());
		usMemberPersistence.remove(member);
	}
}